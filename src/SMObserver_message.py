...
class Obsrv():
    def __init__(self):
        ...
        sub_Enc = message_filters.Subscriber('encoder',Odometry,queue_size = 10)
        sub_Imu = message_filters.Subscriber('imu_k64',Imu,queue_size = 10)
        sub_Imue = message_filters.Subscriber('imu_ext',Imu,queue_size = 10)
        self.Init()
        pub_Obs = rospy.Publisher('Obsrv_message_out',Obsrv_cmd,queue_size=10)
        ts = message_filters.ApproximateTimeSynchronizer([sub_Enc,sub_Imu,sub_Imue],queue_size = 10,slop = 0.05,allow_headerless=True)
        ts.registerCallback(self.Data)
    ...
    def Observer(self,pub_Obs):
        ...
        Chat_new = self.Chat + 0.01*( + h2*numpy.tanh(500*(self.xreal - self.xhat)))
        ...
        Shat_new = self.Shat + 0.01*( + h4*numpy.tanh(500*(self.yreal - self.yhat)))
    ...
    def Data(self,sub_Enc,sub_Imu,sub_Imue):
