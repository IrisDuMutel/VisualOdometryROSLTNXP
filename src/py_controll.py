...
class SMC():
    ...
    def ctr_step(self,sub_APFout, sub_odometry, sub_goal):
        ...
        if math.fabs(psie) > math.pi:
            psie=psie-2*math.pi*numpy.sign(psie)
        ...
        eps1 = 0.3
        eps2 = 0.9
        u1=.9*s1+0.7*s1/(math.fabs(s1)+eps1)+wr*ye+vd*math.cos(psie)
        
        u2=(0.01*s2+0.5*s2/(math.fabs(s2)+eps2)+wd+(ye*vd_dot+vd*vd*math.sin(psie))/(1+(ye*vd)*(ye*vd)))/(1+xe*vd/(1+(ye*vd)*(ye*vd)))
